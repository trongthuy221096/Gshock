...
public class AdminAddReviewController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int vote = Integer.parseInt(request.getParameter("avote"));
		int wid= Integer.parseInt(request.getParameter("awid"));
			
		if("".equals(fullname)|| "".equals(email)|| vote==0 || "".equals(comment) ) {
			response.sendRedirect(request.getContextPath()+"deital?id=9");
			return;
		}else {
			Review review = new Review(0, fullname, email, vote, comment, wid);
			reviewDAO.addItem(review);					
			ArrayList<Review> listReview = reviewDAO.getItemsByWid(wid) ;
			
			int numbervote = reviewDAO.getCountByWid(wid); 
			float result = 0;
			for(Review itemreview :listReview) {
				result+=itemreview.getVote();
			}
			float c = result/numbervote;
			float resultvote = ((float) Math.floor(c * 10) / 10);
			ArrayList<Comment> listCmt = cmtDAO.getItems();
			watchDAO.Updatevote(numbervote,resultvote,wid);
